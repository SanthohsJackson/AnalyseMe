...
```
**Intent:** Resolve internal dependencies by matching imports to existing files in the repository.
...
- None
**Side effects:**
- Modifies FileMeta.depends_on for each file in the list

### `_exists` (function)
```
_exists(base: str, exts: tuple[str, ...]) -> str | None
```
**Intent:** Check if a file with any of the given extensions exists at the base path.

**Inputs:**
- base: str — base path to check for existence
- exts: tuple[str, ...] — possible file extensions
**Outputs:**
...
```
**Intent:** Classify the kind of file based on its path and extension.
...
```
**Intent:** Parse a Node.js package.json file to extract metadata and dependencies.

**Inputs:**
- path: str — path to the package.json file
**Outputs:**
- dict — dictionary containing type, name, version, dependencies, and devDependencies
...
```
**Intent:** Parse a Java pom.xml file to extract dependencies.
